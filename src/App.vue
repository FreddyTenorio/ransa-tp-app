  ...
  <div :class="{ 'sorting-active': sortingActive }">
    <div class="top-panel">
      <div class="panel-content">
        ...
        <div class="stats-row">
          <div class="stat-item"><span class="stat-val">{{ ruta.length }}</span><span class="stat-lbl">Total</span>
          </div>
          <div class="stat-item"><span class="stat-val" style="color: #17a2b8;">{{ countLoc }}</span><span
              class="stat-lbl">Ubic.</span></div>
          <div class="stat-item"><span class="stat-val" style="color: #28a745;">{{ countDel }}</span><span
              class="stat-lbl">Listos</span></div>
        </div>
        ...
        <div class="search-filter-row" v-show="!sortingActive">
          ...
          <div class="type-chips">
            ...
            <button @click="typeFilter = 'ENTREGA'"
              :class="{ active: typeFilter === 'ENTREGA', 'active-ent': typeFilter === 'ENTREGA' }">Entregas</button>
            <button @click="typeFilter = 'RECOJO'"
              :class="{ active: typeFilter === 'RECOJO', 'active-rec': typeFilter === 'RECOJO' }">Recojos</button>
            <button @click="typeFilter = 'INTERCAMBIO'"
              :class="{ active: typeFilter === 'INTERCAMBIO', 'active-int': typeFilter === 'INTERCAMBIO' }">Interc.</button>
          </div>
        </div>
    ...
    <div id="lista" ref="listaRef" style="padding-top: 240px; padding-bottom: 100px;">
      ...
      <div v-for="(c, index) in ruta" :key="c.guid" class="card"
        ...
        <div class="header">
          ...
          <div style="flex-grow:1; padding-right:10px;">
            ...
            <span class="phone">📞 {{ c.tel }} <button style="background:none;border:none;cursor:pointer;"
                @click="editPhone(c)">✏️</button></span>
            <div style="font-size: 0.8em; color: #6c757d; margin-top: 2px;">📄 Guía: {{ c.guia }}</div>
          </div>
        ...
        <div class="report-row">
          <a class="btn-report-issue btn-nowa" :href="getLinkNoWa(c)" target="_blank">🚫 Sin WA</a>
          <a class="btn-report-issue btn-wrong-num" :href="getLinkWrongNum(c)" target="_blank">📞❌ Num Error</a>
          <a class="btn-report-issue btn-bad-loc" :href="getLinkBadLoc(c)" target="_blank">📍❌ Ubic Mal</a>
          <a class="btn-report-issue btn-no-resp" :href="getLinkNoResp(c)" target="_blank">🔕 No Resp.</a>
          <a class="btn-report-issue btn-rechazo" :href="getLinkRechazo(c)" target="_blank" @click="marcarRechazo(c)">⛔
            Rechazo</a>
        </div>
        ...
        <div class="loc-section">
          ...
          <input type="text" class="input-link" placeholder="Pegar Link Maps..." v-model="c.link"
            @input="handleLinkInput(c)">
          <a class="btn-report-base" :class="{ ready: c.link }"
            :href="c.link ? `https://wa.me/${numeroBase}?text=${encodeURIComponent('Ubicación de ' + c.nom + ': ' + c.link)}` : '#'"
            target="_blank">
            👮 Reportar Ubicación
          </a>
        </div>
    ...
    <div class="modal-overlay" :class="{ open: modalOpen }">
      <div class="modal-content">
        ...
        <div style="display:flex; gap:10px; margin-top:10px;">
          <button
            style="flex:1; padding:12px; border-radius:8px; border:none; font-weight:bold; cursor:pointer; background:#dc3545; color:white;"
            @click="modalOpen = false">Cancelar</button>
          <button
            style="flex:1; padding:12px; border-radius:8px; border:none; font-weight:bold; cursor:pointer; background:#0d6efd; color:white;"
            @click="saveModalChanges">💾 Guardar</button>
        </div>
      </div>
    </div>
    ...
    <div class="slide-menu" :class="{ open: menuOpen }">
      ...
      <button class="btn-upload-label" style="background:#dc3545;" @click="clearAllData">🗑️ Borrar Ruta de la
        Nube</button>
    </div>
  </div>
</template>
    ...
    try {
      ...
      workbook.SheetNames.forEach(sheetName => {
        ...
        json.forEach(row => {
          ...
          const telefonoRaw = String(row.TELEFONO || row.Telefono || "");
          const cleanTel = telefonoRaw.replace(/\D/g, "");
          ...
          const departamentoRaw = row.DEPARTAMENTO || row.Departamento || "LIMA";
          ...
          newRuta.push({
            ...
            distritoRaw: distritoRaw,
            departamentoRaw: departamentoRaw,
            tel: finalTel,
            desc: row.MOTIVO || row.Motivo || "",
            cc: row.CC || row.Cc || "",
            tipo: tipo,
            loc: false,
            link: "",
...
*DNI:* ${c.dni}
*Cliente:* ${c.nom}
*Dirección:* ${c.direccionRaw}
*Distrito:* ${c.distritoRaw}
*Departamento:* ${c.departamentoRaw}
*Teléfono:* ${c.tel}
*Motivo Original:* ${c.desc}
*CC:* ${c.cc}

*Razon:* `;
  return `https://wa.me/${numeroBase}?text=${encodeURIComponent(msg)}`;
};
...
const applyFilters = () => { /* Vue reactividad hace esto automático */ };
...
.type-chips button.active-ent {
  background: #0d6efd;
  color: white;
}

.type-chips button.active-rec {
  background: #6f42c1;
  color: white;
}

.type-chips button.active-int {
  background: #fd7e14;
  color: white;
}
...
.sorting-active .header-right,
.sorting-active .actions,
.sorting-active .report-row,
.sorting-active .loc-section,
.sorting-active .btn-done,
.sorting-active .phone,
.sorting-active .card-info {
  display: none !important;
}
